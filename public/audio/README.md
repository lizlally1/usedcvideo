...
- `voiceover.mp3` — the narration track (see voiceover-script.txt at the
  ...
  configured via `.env`) or `npm run voiceover:fallback` (offline
  espeak-ng placeholder), or drop in a manually produced/recorded file.
- `music.mp3` — the background instrumental bed. Generate a synthesized
  ...
  licensed/royalty-free soft instrumental-country track.
